...
void show_about_dialog(GtkButton *button, gpointer none)
{
    ...
    gtk_about_dialog_set_version(GTK_ABOUT_DIALOG(dialog), "v0.1 beta");
    gtk_about_dialog_set_comments(GTK_ABOUT_DIALOG(dialog), "A test project");
...
gboolean search_entry_key_press(GtkEntry *entry, GdkEvent *event, gpointer none)
{
    ...
    //else if ((keyEvent->keyval != GDK_KEY_BackSpace) && (keyEvent->keyval != GDK_KEY_Enter) && (keyEvent->keyval != 65364) && (keyEvent->keyval != 65362)){ //Nếu kí tự  không là backspace,enter,mũi tên lên xuống
    else {
        if (keyEvent->keyval != GDK_KEY_BackSpace){
            length = strlen(word);
            if ((keyEvent->keyval != 65364) && (keyEvent->keyval != 65362)){
                word[length] = keyEvent->keyval;
            }           
		    word[length + 1] = '\0';
        }
        else {
...
void show_completion(char *word)
{
	...
	GtkTreeIter Iter;
	int existed, value, index;
	int number = 10; // so luong tu goi y

    gtk_list_store_clear(liststore);

	existed = bfndky(data, word, &value); // ham tim kiem word trong file data
	if (existed == 0)
	{
	...
	{
		if (bnxtky(data, suggest_word, &value) == 0) // Tim kiem tu tiep theo cua word trong tu dien
		{
			if (prefix(suggest_word, word) == 0)
			{
